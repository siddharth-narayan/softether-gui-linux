...
<div class="flex flex-col bg-primary p-8 gap-4 text-text font-bold w-full h-full">
    ...
    {#if settings.length === 0}
        ...
        {#each settings as setting}
            {#if setting.required == true}
                <Setting {setting} bind:array={array} index={indexf()}></Setting>
            {/if}
        {/each}
    {/await}
